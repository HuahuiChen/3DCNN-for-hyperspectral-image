...
template <typename Dtype>
void Net<Dtype>::Init(const NetParameter& in_param) {//net初始化，设置各层layer
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    for (int top_id = 0; top_id < num_top; ++top_id) {
      AppendTop(param, layer_id, top_id, &available_blobs, &blob_name_to_idx);//数据在这里读入??
    ...
    if (share_from_root) {
      ...
      layers_[layer_id]->SetUp(bottom_vecs_[layer_id], top_vecs_[layer_id]);//设置各层layer
    }
    LOG_IF(INFO, Caffe::root_solver())
